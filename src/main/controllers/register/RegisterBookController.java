...
public class RegisterBookController implements Initializable {
	...
	private void regjistroButtonClicked(ActionEvent event) throws Exception {
		if (emriLibritAutoriField.getText().isEmpty() || vitiBotimitField.getText()
			...
			labSuccess.setText("Nuk lejohen shkronja, tek ISBNKodi!");
			labSuccess.setStyle("-fx-text-fill: #FF073A;");
		} else if (sasiaField.getText().matches("^[a-zA-Z\\s]+$")) {
			labSuccess.setText("Nuk lejohen shkronja, tek Sasia!");
			labSuccess.setStyle("-fx-text-fill: #FF073A;");
		} else if (vitiBotimitField.getText().matches("^[a-zA-Z\\s]+$")) {
			labSuccess.setText("Nuk lejohen shkronja, tek Viti i Botimit!");
			labSuccess.setStyle("-fx-text-fill: #FF073A;");
		} else if (vitiBotimitField.getText().matches("^[a-zA-Z\\s]+$")
				&& sasiaField.getText().matches("^[a-zA-Z\\s]+$")
				&& ISBNKodiField.getText().matches("^[a-zA-Z\\s]+$")) {
			labSuccess.setText("Nuk lejohen shkronja, tek Sasia, ISBNKodi dhe Viti i Botimit!");
			labSuccess.setStyle("-fx-text-fill: #FF073A;");
		} else if (sasiaField.getText().matches("^[a-zA-Z\\s]+$")
				&& ISBNKodiField.getText().matches("^[a-zA-Z\\s]+$")) {
			labSuccess.setText("Nuk lejohen shkronja, tek Sasia, ISBNKodi!");
			labSuccess.setStyle("-fx-text-fill: #FF073A;");
		} else if (vitiBotimitField.getText().matches("^[a-zA-Z\\s]+$")
				&& sasiaField.getText().matches("^[a-zA-Z\\s]+$")) {
			labSuccess.setText("Nuk lejohen shkronja, tek Sasia, Viti i Botimit!");
			labSuccess.setStyle("-fx-text-fill: #FF073A;");
		}
		else {
				checkNeseEgziston();
