...
public class UpdateVendinController implements Initializable {
	...
	private Vendet getVendetFromUI() {
		int nrVendi = Integer.parseInt(nrUlsesField.getText());
		String EmriMbiemri = emriMbiemriField.getText();
		String Sektori = sektoriField.getText();
		Timestamp DataKoha = Timestamp.valueOf(dateKohaField.getText());

		return new Vendet(nrVendi, EmriMbiemri, Sektori, DataKoha);
	}

	private void updateVendet(int nrVendi, String EmriMbiemri, String Sektori, Timestamp DataKoha) throws Exception {
		Connection con = DBConnector.getConnection();
		String sql = "Update Vendet set nrVendi = ?, EmriMbiemri = ?, Sektori = ?, DataKoha = ? Where EmriMbiemri = ?";
		PreparedStatement stmt = con.prepareStatement(sql);

		stmt.setInt(1, nrVendi);
		stmt.setString(2, EmriMbiemri);
		stmt.setString(3, Sektori);
		stmt.setTimestamp(4, DataKoha);
		stmt.setString(5, EmriMbiemri);
	...
	private void check() throws Exception {
		if (nrUlsesField.getText().isEmpty() || emriMbiemriField.getText().isEmpty()
		...
		} else if ((Integer.parseInt(nrUlsesField
				.getText()) >= 1
				&& Integer.parseInt(nrUlsesField
						.getText()) <= 100
			...
			emptyField();
		} else if ((Integer.parseInt(nrUlsesField.getText()) >= 101
				&& Integer.parseInt(nrUlsesField.getText()) <= 200
				&& sektoriField.getText().equals("Kati i dyte"))) {
			update();
			labUserInfo.setText("Update Succesfully!");
			labUserInfo.setStyle("-fx-text-fill: #2DFE54;");
			emptyField();
		} else if ((Integer.parseInt(nrUlsesField.getText()) >= 201
				&& Integer.parseInt(nrUlsesField.getText()) <= 300
				&& sektoriField.getText().equals("Kati i trete"))) {
			update();
			labUserInfo.setText("Update Succesfully!");
			labUserInfo.setStyle("-fx-text-fill: #2DFE54;");
			emptyField();
		} else if ((Integer.parseInt(nrUlsesField.getText()) >= 1
				&& Integer.parseInt(nrUlsesField.getText()) <= 100
				&& sektoriField.getText().equals("Kati i dyte"))) {
	...
	private void update() {
		try {
			Vendet selected = tableview.getSelectionModel().getSelectedItem();
			Vendet changed = getVendetFromUI();

			updateVendet(changed.getNrVendi(), changed.getEmriMbiemri(), changed.getSektori(), changed.getDataKoha());
			selected.setNrVendi(changed.getNrVendi());
			selected.setEmriMbiemri(changed.getEmriMbiemri());
			selected.setSektori(changed.getSektori());
			selected.setDataKoha(changed.getDataKoha());
